...
typedef struct {
    ...
    Module modules[6];
    float gpa;
    char* highest_module; // Change data type to char*
    char* lowest_module;  // Change data type to char*
...
void calculate_highest_scoring_module(Student* student) {
    ...
    if (max_module_index >= 0) {
        ...
        student->highest_module = (char*)malloc(strlen(student->modules[max_module_index].name) + 1);
...
void calculate_lowest_scoring_module(Student* student) {
    ...
    if (min_module_index >= 0) {
        // Allocate memory for the module name and copy it
        student->lowest_module = (char*)malloc(strlen(student->modules[min_module_index].name) + 1);
...
// Function to free dynamically allocated memory for module names
void free_memory(Student* students, int num_students) {
    for (int i = 0; i < num_students; i++) {
        free(students[i].highest_module);
        free(students[i].lowest_module);
    }
}
...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        printf("%-25s%.2f\t%-25s%-25s%.2f\t%.2f\t%.2f\n", students[i].name, students[i].gpa,
    ...
    // Free dynamically allocated memory for module names
    free_memory(students, 10);
